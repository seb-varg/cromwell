...
class BigQueryReporter(override val params: ErrorReporterParams) extends ErrorReporter {
  ...
  private def toTestFailureRow(testEnvironment: TestEnvironment,
    ...
    RowToInsert of Map(
      ...
      "test_timestamp" -> Option(OffsetDateTime.now.asUtcString),
  ...
  private def toCallAttemptFailureRow(callAttemptFailure: CallAttemptFailure): RowToInsert = {
    RowToInsert of Map(
      ...
      "end" -> callAttemptFailure.endOption.map(_.asUtcString),
      ...
      "start" -> callAttemptFailure.startOption.map(_.asUtcString),
  ...
  private def toMetadataRow(metadataEntry: MetadataEntry): RowToInsert = {
    RowToInsert of Map(
      ...
      "metadata_timestamp" -> Option(metadataEntry.metadataTimestamp.toSystemOffsetDateTime.asUtcString),
